...
namespace _2ClassSVMUtil
{
    public class twoClassSVMUtil : IMachineLearning
    {
        ...
        public void ConvertInputParams(int nSV, double weight)
        {
            ColumnVector f_hat = new ColumnVector(_UnknownData_Node.Length);

            //印象変換ベクトルを取得。
            ColumnVector cSV = MakeITV(nSV);
            ...
            {
                f_hat[i] = _UnknownData_Node[i].Value + weight * dist * cSV[i];
            }

            EditParam = f_hat;
        }

        private ColumnVector MakeITV(int nSV)
        {
            Node[][] tVectors = new Node[nSV][];
            Node[][] fVectors = new Node[nSV][];

            int st = 0;
            int sf = model.NumberOfSVPerClass[0];
            int ct = model.NumberOfSVPerClass[0];
            int cf = model.NumberOfSVPerClass[1];
  
            //全ノードの距離計算を行います。添え字はサポートベクタの番号と対応
            double[] dist = new double[model.SupportVectorCount];

            for (int i = 0; i < model.SupportVectorCount; i++)
                dist[i] = EuclideanDistance_between_Nodes(_UnknownData_Node, model.SupportVectors[i]);

            //distを正解クラスと不正解クラスに分けて最小距離ランキングづけします。
            int[] key = new int[nSV];
            double[] minDist = new double[nSV];
            ...
            for (int j = 0; j < nSV; j++)
            {
                tVectors[j] = model.SupportVectors[key[j]];
            ...
            for (int j = 0; j < nSV; j++)
            {
                fVectors[j] = model.SupportVectors[key[j]];
            }

            //重心の計算をして、方向ベクトルを返します。次元数は入力から最も近いサポートベクタに合わせます。
            ColumnVector gt = new ColumnVector(tVectors[0].Length);
            ColumnVector gf = new ColumnVector(fVectors[0].Length);

            for (int i = 0; i < nSV; i++)
            {
                for (int j = 0; j < tVectors[i].Length; j++)
                {
                    gt[j] += tVectors[i][j].Value;
                }

                for (int j = 0; j < fVectors[i].Length; j++)
                {
                    gf[j] += fVectors[i][j].Value;
                }
            }
            gt /= tVectors.Length;
            gf /= fVectors.Length;
            ...
            //正クラスの方を向かせるので
            ColumnVector rtnVector = gt - gf;

            //大きさ1に正規化
            double scale = 0;
            for (int j = 0; j < rtnVector.Length; j++)
            {
                scale += Math.Pow(rtnVector[j], 2);
            }
            scale = Math.Sqrt(scale);

            for (int k = 0; k < rtnVector.Length; k++)
            {
                rtnVector[k] /= scale;
            }
            return rtnVector;
